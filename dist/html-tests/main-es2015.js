...
let AdminPageComponent = class AdminPageComponent {
    ...
    ngOnInit() {
        this.dataService.getResults().then(data => {
            this.results = data;
        });
    }
    getPercents(answers) {
        const total = answers.reduce((accum, item) => accum += item.cost, 0);
...
let CompletePageComponent = class CompletePageComponent {
    constructor(dataService, router) {
        this.dataService = dataService;
        this.router = router;
    }
...
CompletePageComponent.ctorParameters = () => [
    { type: _services_data_service__WEBPACK_IMPORTED_MODULE_2__["DataService"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_3__["Router"] }
];
